...
func (apiCfg *apiConfig) handlerCreateFeedFollow(w http.ResponseWriter, r *http.Request, user database.User){
	...
	respondWithJSON(w,201,databaseFeedToFollow(feedFollow))
}
...
func (apiCfg *apiConfig) handlerGetFeedFollow(w http.ResponseWriter, r *http.Request, user database.User){
	...
	if err!=nil{
		respondWithError(w,400,fmt.Sprintf("Couldn't get the feeds which are followed: %v",err))
		return
	}
	respondWithJSON(w,200,databaseFeedFollowToUser(feedFollow))
}
